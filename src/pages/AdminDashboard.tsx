        ...
        <div className="bg-white border-b border-slate-200 px-6 py-3.5 flex flex-col sm:flex-row sm:justify-between sm:items-center sticky top-0 z-10 gap-3">
            ...
            <div className="flex items-center space-x-2">
              ...
              <button
                ...
                title="Demo Sandbox & Testing Options"
              >
        ...
        <div className="fixed inset-0 z-50 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl max-w-sm w-full p-6 border border-slate-100 relative animate-in fade-in zoom-in-95">
            ...
            <p className="text-xs text-slate-500 mb-4">
              Quickly switch workspace perspective or test resident features.
            </p>
            <div className="space-y-2.5">
              <button
                onClick={async () => {
                  if (user?.uid) {
                    await updateDoc(doc(db, "users", user.uid), { role: "resident" });
                    setShowDemoModal(false);
                  }
                }}
                className="w-full text-left p-3 rounded-xl border border-slate-200 hover:border-indigo-500 hover:bg-indigo-50/50 transition flex items-center justify-between group cursor-pointer"
              >
                <div>
                  <h4 className="text-xs font-bold text-slate-900 group-hover:text-indigo-700">Switch to Resident View</h4>
                  <p className="text-[11px] text-slate-500">Submit maintenance complaints, view notice boards, and invite household members</p>
                </div>
                <span className="text-xs font-bold text-indigo-600">Switch &rarr;</span>
              </button>
            </div>
            <button
